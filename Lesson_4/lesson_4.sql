...
WHERE EXISTS (
    ...
    WHERE fr.status LIKE 'approved' AND u.id = 1);
...
WHERE u.id IN (
		...
		FROM friend_requests fr
		WHERE target_user_id = 1 AND fr.status LIKE 'approved'
	UNION
		SELECT target_user_id
		FROM friend_requests fr
		WHERE initiator_user_id = 1 AND fr.status LIKE 'approved')
GROUP BY u.id
HAVING COUNT(u.id) = MAX(u.id);
